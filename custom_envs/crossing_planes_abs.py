...
class Cross_env(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.total_steer_action_magnitude = 0.0
        self.total_action_age = 0.0
    ...
    def step(self, action):
        ...
        # Track kumulative Action Age
        self.total_action_age += aktiv_agent.action_age
    ...
    def _compute_action_history_refactored(self, agent: Agent) -> np.ndarray:
        """
        REFACTORED Action History (7 features):
        ...
        - last_target_heading_cos: [-1, 1] - Letztes Ziel-Heading (cos)
        - last_target_heading_sin: [-1, 1] - Letztes Ziel-Heading (sin)
        - target_heading_normalized: [-1, 1] - Differenz zwischen letztem target_heading und aktuellem Heading
        """
        ...
        # Letztes Target Heading als cos/sin + Differenz zum aktuellen Heading
        ac_hdg = agent.ac_hdg
        if agent.target_heading is not None:
            target_hdg_rad = np.deg2rad(agent.target_heading)
            last_target_heading_cos = np.cos(target_hdg_rad)
            last_target_heading_sin = np.sin(target_hdg_rad)
            
            target_heading_diff = fn.bound_angle_positive_negative_180(agent.target_heading - ac_hdg)
            target_heading_normalized = np.clip(target_heading_diff / 180.0, -1.0, 1.0)
        else:
            # Kein target_heading gesetzt → verwende aktuelles Heading als Default
            ac_hdg_rad = np.deg2rad(ac_hdg)
            last_target_heading_cos = np.cos(ac_hdg_rad)
            last_target_heading_sin = np.sin(ac_hdg_rad)
            target_heading_normalized = 0.0
        ...
        return np.array([
            ...
            last_target_heading_cos,
            last_target_heading_sin,
            target_heading_normalized
        ], dtype=np.float32)
    ...
    def _get_observation_dict(self, agent: Optional[Agent] = None) -> Dict[str, np.ndarray]:
        ...
        Returns dict mit 4 Komponenten:
        ...
        - action_history: (7) Last Action, Age, Turn Rate, Last Target Heading (cos/sin + normalized)
    ...
    def _get_reward(self):
        ...
        5. action_penalty: [-ACTION_PENALTY, 0] (Proportional zu Aktionsgröße) ← GRADIENT LEARNING!
           - Kleine Aktionen (≈0) → fast keine Penalty
           - Große Aktionen (±1) → volle Penalty
        ...
        - Action Penalty: Bestraft proportional → Agent lernt "so klein wie möglich, so groß wie nötig"
        ...
        - Smooth gradient für Action-Größe → SAC kann optimal lernen
        """
        agent = self.all_agents.get_active_agent()
        ...
        # === ACTION PENALTY ===
        # Proportionale Strafe basierend auf Aktionsgröße → Agent lernt GRADIENT!
        # Kleine Aktionen (0.05) → kleine Penalty (-0.015)
        # Große Aktionen (1.0) → volle Penalty (-0.3)
        action_penalty = -ACTION_PENALTY * abs(agent.last_action_continuous)
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"[Step {self.steps}] ACTION PENALTY: {action_penalty:+.3f} (action_magnitude={abs(agent.last_action_continuous):.3f})")
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"[Step {self.steps}] REWARDS: "
                        ...
                        f"ActionPenalty={action_penalty:+.3f} | "
        ...
        self.cumulative_action_penalty += action_penalty
    ...
    def _set_action(self, action, agent: Agent) -> None:
        """
        Verarbeitet Continuous Action: [target_heading_change]
        target_heading_change: [-1, 1] → Neues Ziel-Heading relativ zu aktuellem Heading
        
        NOOP-Logik (algorithmisch):
        - Wenn neues Ziel-Heading ≈ gespeichertes agent.target_heading → NOOP
        - Wenn neues Ziel-Heading ≠ gespeichertes agent.target_heading → ACTION (Steering)
        """
        target_heading_change = float(action[0])  # [-1, 1]
        
        # Clip to valid range
        target_heading_change = np.clip(target_heading_change, -1.0, 1.0)
        
        # Get current state
        _, _, current_heading, _ = self.sim.traf_get_state(agent.id)
        
        # Berechne neues gewünschtes Ziel-Heading
        dh = target_heading_change * D_HEADING
        new_target_heading = fn.bound_angle_positive_negative_180(current_heading + dh)
        
        # ALGORITHMISCHE NOOP-ENTSCHEIDUNG
        if agent.target_heading is None:
            # Erstes Mal: Setze target_heading
            agent.target_heading = new_target_heading
            is_noop = False
        else:
            # Prüfe ob neues Ziel-Heading vom gespeicherten abweicht
            heading_diff = abs(fn.bound_angle_positive_negative_180(new_target_heading - agent.target_heading))
            
            if heading_diff > 0.3:  # Threshold: 1° Abweichung
                # Neues Ziel → ACTION
                agent.target_heading = new_target_heading
                is_noop = False
            else:
        ...
        # Update Agent State
        agent.is_noop = is_noop
        agent.last_action_continuous = target_heading_change
        self.last_continuous_action = target_heading_change
        ...
        # Track action magnitude (für alle Actions, inkl. NOOPs)
        action_magnitude = abs(target_heading_change)
        if is_active_agent:
        ...
        if is_noop:
            ...
            if is_active_agent:
                ...
                self.total_steer_action_magnitude += action_magnitude
            ...
            if self.render_mode is not None and x_pos is not None:
                if agent.id in self.action_markers:
                    ...
                    if len(self.action_markers[agent.id]) > MAX_ACTION_MARKERS:
                        self.action_markers[agent.id].pop(0)
                if hasattr(agent, 'action_markers_with_steering'):
                    agent.action_markers_with_steering.append((x_pos, y_pos, 'steer', target_heading_change, agent.action_age))
                    if len(agent.action_markers_with_steering) > MAX_ACTION_MARKERS:
                        agent.action_markers_with_steering.pop(0)
            
            # Setze neues Heading im Simulator
            self.sim.traf_set_heading(agent.id, agent.target_heading)
            agent.last_set_heading = agent.target_heading
            agent.action_age = 0
    ...
    def _render_frame(self):
        ...
        for agent in self.all_agents.get_all_agents():
            ...
            # Zeige absolutes Ziel-Heading (target_heading) wenn gesetzt
            if agent.target_heading is not None:
                target_hdg_text = f"TGT: {agent.target_heading:.0f}°"
                target_text_surface = font.render(target_hdg_text, True, (0, 150, 0))
                target_text_x = x_pos + radius_px + 5
                target_text_y = y_pos + 10
                canvas.blit(target_text_surface, (int(target_text_x), int(target_text_y)))
        ...
        if active_agent:
            ...
            action_penalty_r = reward_comp.get('action_penalty', 0.0)
            ...
            # ActionPen ist immer ≤ 0 (Strafe), daher: grün wenn 0 (kein Penalty), rot wenn negativ
            text = f"ActionPen:    {action_penalty_r:+.4f}"
            color = (100, 100, 100) if action_penalty_r == 0 else (200, 0, 0)
            text_surf = font_normal.render(text, True, color)
            canvas.blit(text_surf, (debug_x_right, debug_y_right))
            debug_y_right += 20
