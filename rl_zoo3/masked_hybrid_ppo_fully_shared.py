...
MaskedSelfHybridPPO = TypeVar("MaskedSelfHybridPPO", bound="MaskedHybridFullySharedPPO")
...
class SplitActionNet(th.nn.Module):
    def __init__(self, latent_dim: int, net_action_dims: List[int], action_space:  spaces.Dict, net_arch: Union[list[int], dict[str, list[int]]] = None):
        super().__init__()
        
        if net_arch is None:
            # 2 shared layers with 64 neurons, no individual branches (empty lists)
            net_arch = {
                'shared': [64, 64],
                'main_opt_switch': [],
                'main_val_power': [],
        ...
        print(f"Initializing SharedSplitActionNet (Fully Shared) with net_arch: {net_arch} \nand action_space: {action_space}\n and latent_dim: {latent_dim} \n and net_action_dims: {net_action_dims}")
        
        # Shared Network (Trunk)
        self.shared_net = th.nn.Sequential()
        curr_dim = latent_dim
        
        if isinstance(net_arch, dict) and 'shared' in net_arch:
            for hidden_dim in net_arch['shared']:
                self.shared_net.append(th.nn.Linear(curr_dim, hidden_dim))
                self.shared_net.append(th.nn.Tanh())
                curr_dim = hidden_dim

        self.action_nets = th.nn.ModuleList()
        if isinstance(net_arch, dict):
            # Check if all action keys are present
            assert all(key in net_arch for key in action_space.keys()), \
                f"net_arch missing keys. Needed: {list(action_space.keys())}, Found: {list(net_arch.keys())}"

            for key, action_dim in action_space.items():
                layers = []
                branch_input_dim = curr_dim
                current_net_arch = net_arch[key]
                for hidden_dim in current_net_arch:
                    layers.append(th.nn.Linear(branch_input_dim, hidden_dim))
                    layers.append(th.nn.Tanh())
                    branch_input_dim = hidden_dim
                layers.append(th.nn.Linear(branch_input_dim, get_net_action_dim(action_dim)))
                self.action_nets.append(th.nn.Sequential(*layers))
        else:
            # Fallback for list: Create independent networks for each action with the same architecture
        ...
        print( f"Constructed SharedSplitActionNet (Fully Shared) with {self.action_nets} action nets." )


    def forward(self, x: th.Tensor) -> th.Tensor:
        features = self.shared_net(x)
        return th.cat([action_net(features) for action_net in self.action_nets], dim=1)
...
class Masked_Hybrid_MultiOutputActorCriticPolicy(MultiOutputActorCriticPolicy):
    def __init__(
        ...
        # SB3 initializes the *entire* action_net with gain=0.01. 
        # Since SplitActionNet contains deep hidden layers, this kills the gradients.
        # We must re-initialize the hidden layers with gain=sqrt(2) (orthogonal) for Tanh,
        # and keep only the final output layers at gain=0.01.
        if isinstance(self.action_net, SplitActionNet):
            module = self.action_net
            
            # 1. Initialize Shared Trunk (Hidden Layers)
            for name, m in module.shared_net.named_modules():
...
class MaskedHybridFullySharedPPO(MultiOutputPPO):
    ...
    def learn(
            ...
            log_interval: int = 1,
            tb_log_name: str = "Masked_Hybrid_PPO_fully_shared",
