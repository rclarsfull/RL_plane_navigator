...
MaskedSelfHybridPPO = TypeVar("MaskedSelfHybridPPO", bound="MaskedHybridSharedPPO")
...
class SplitActionNet(th.nn.Module):
    def __init__(self, latent_dim: int, net_action_dims: List[int], action_space:  spaces.Dict, net_arch: Union[list[int], dict[str, list[int]]] = None):
        ...
        print( f"Constructed SharedSplitActionNet with {self.action_nets} action nets." )
...
class Masked_Hybrid_MultiOutputActorCriticPolicy(MultiOutputActorCriticPolicy):
    def __init__(
        ...
        # --- Fix for SB3 Initialization (Deep Action Net) ---
        if isinstance(self.action_net, SplitActionNet):
            # 1. Initialize Shared Trunk (if exists)
            if hasattr(self.action_net, 'shared_net'):
                for name, m in self.action_net.shared_net.named_modules():
            ...
            if hasattr(self.action_net, 'action_nets'):
                for branch in self.action_net.action_nets:
                    ...
                    if len(layers) > 0:
                        ...
                        # Final output layer -> gain=0.01 (SB3 default for action net)
                        last_m = layers[-1]
...
class MaskedHybridSharedPPO(MultiOutputPPO):
    ...
    def learn(
            ...
            log_interval: int = 1,
            tb_log_name: str = "Masked_Hybrid_PPO_shared_split",
