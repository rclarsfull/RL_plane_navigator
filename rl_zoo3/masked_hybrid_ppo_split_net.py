...
#    def __init__(self, latent_dim: int, net_action_dims: List[int], net_arch: List[int] = [64, 32]):
#        super().__init__()
#        
#        # Erste Dimension ist der gemeinsame Layer
#        shared_dim = net_arch[0]
#        self.shared_net = th.nn.Sequential(
#            th.nn.Linear(latent_dim, shared_dim),
#            th.nn.Tanh()
#        )
#        
#        # Die restlichen Dimensionen sind für die getrennten Heads
#        head_dims = net_arch[1:]
#        
#        self.heads = th.nn.ModuleList()
#        for action_dim in net_action_dims:
#            layers = []
#            curr_dim = shared_dim
...
#                curr_dim = hidden_dim
#            layers.append(th.nn.Linear(curr_dim, action_dim))
#            self.heads.append(th.nn.Sequential(*layers))
#
#    def forward(self, x: th.Tensor) -> th.Tensor:
#        shared_out = self.shared_net(x)
#        return th.cat([head(shared_out) for head in self.heads], dim=1)
...
class SplitActionNet(th.nn.Module):
    def __init__(self, latent_dim: int, net_action_dims: List[int], action_space:  spaces.Dict, net_arch: Union[list[int], dict[str, list[int]]] = dict({
            'main_opt_switch': [32, 32],
            'main_val_power': [32, 32],
            'side_opt_switch': [32, 32],
            'side_val_power': [32, 32]
        })):
        super().__init__()
...
class Masked_Hybrid_MultiOutputActorCriticPolicy(MultiOutputActorCriticPolicy):
    def __init__(
        ...
        if isinstance(self.action_net, SplitActionNet):
            # 1. Initialize Shared Trunk (if exists - unlikely for this class but safe)
            if hasattr(self.action_net, 'shared_net') and isinstance(self.action_net.shared_net, th.nn.Module):
...
class MaskedHybridPPO(MultiOutputPPO):
    ...
    def learn(
            ...
            log_interval: int = 1,
            tb_log_name: str = "Masked_Hybrid_PPO_split_net",
